...
int oplus_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	rc = utils->read_u32(utils->data, "oplus,hpwm-onepulse-bl-threshold", &val);
	if (rc) {
		panel->oplus_priv.hpwm_onepulse_support = false;
	} else {
		panel->bl_config.hpwm_onepulse_bl_threshold = val;
	}
	LCD_INFO("[%s] oplus,hpwm-onepulse-bl-threshold=%d\n",
			panel->oplus_priv.vendor_name,
			panel->bl_config.hpwm_onepulse_bl_threshold);
...
int oplus_panel_global_hbm_mapping(struct dsi_panel *panel, u32 *backlight_level)
{
	...
	if (panel->oplus_priv.hpwm_onepulse_support && panel->oplus_priv.pwm_turbo_enabled) {
		if (bl_lvl > panel->bl_config.pwm_turbo_gamma_bl_threshold
				&& bl_lvl < panel->bl_config.hpwm_onepulse_bl_threshold) {
			bl_lvl = panel->bl_config.pwm_turbo_gamma_bl_threshold +1;
		} else if (bl_lvl >= panel->bl_config.hpwm_onepulse_bl_threshold
				&& bl_lvl <= panel->bl_config.brightness_normal_max_level) {
			bl_lvl = backlight_hpwm_onepulse_normal_buf[bl_lvl - panel->bl_config.hpwm_onepulse_bl_threshold];
		} else if (bl_lvl > panel->bl_config.brightness_normal_max_level) {
			bl_lvl = backlight_hpwm_onepulse_max_buf[bl_lvl - panel->bl_config.brightness_normal_max_level - 1];
		}
	}
	hpwm_onepulse_bl_lvl = bl_lvl;
...
int oplus_panel_pwm_switch_wait_te_tx_cmd(struct dsi_panel *panel, u32 pwm_switch_cmd, u32 pwm_switch_state_last)
{
	...
	if (pwm_switch_state_last != panel->oplus_pwm_switch_state) {
		...
		if (oplus_panel_pwm_onepulse_is_enabled(panel)
				|| (panel->oplus_priv.hpwm_onepulse_support && oplus_panel_pwm_turbo_is_enabled(panel))) {
			if (refresh_rate == 90) {
				oplus_need_to_sync_te(panel);
			}
		}
